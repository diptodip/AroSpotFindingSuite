...
currentZIndices=(sortedSpotData.xyz(:,3)==currentZ);
spotNum=sum(currentZIndices);
...
for si=1:spotNum%should be in order
    %bs(sls.rows(si),sls.cols(si))=sls.scaledValues(si);
    bs(sls.xyz(si,1),sls.xyz(si,2))=sls.scaledValues(si);
end;
blueSpots=sls.xyz(:,1:2);%[sls.rows sls.cols];%%%make sure this is nx2
blueSpotRankList=1:spotNum;
...
%segments=handles.dataStructure.segMasks;
...
function [slice,spotVSortedScaled]=loadAndScaleSlice(segStacks,iSlice,handles)
%disp(iSlice);
spotVSortedScaled=handles.sortedSpotData.scaledValues;
slice=segStacks(:,:,iSlice);
mnSlice=min(slice(slice~=0));
mxSlice=max(slice(slice~=0));
rangeSlice=mxSlice-mnSlice;
slice(slice~=0)=(slice(slice~=0)-mnSlice)/rangeSlice;
spotVSortedScaled(handles.sortedSpotData.xyz(:,3)==iSlice)=((spotVSortedScaled(handles.sortedSpotData.xyz(:,3)==iSlice))-mnSlice)/rangeSlice;
